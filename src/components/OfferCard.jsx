...
function OfferCard({ name, image, price, description, id, discount, isOffer }) {
  ...
  return (
    ...
    <div className="md:max-w-md bg-primary flex flex-col items-center gap-2 p-8 rounded-xl text-center text-gray-300 ">
      {/* Imagen de cada comida y precio en descuento */}
      <div className="w-full flex flex-grow justify-evenly">
      <img
        className="w-36 object-cover shadow-2xl "
        src={image}
        alt={name}
      />
      {/* Precio en descuento */}
      ...
      <h3 className="text-3xl font-semibold text-secondary flex">{discountedPrice}</h3>
      <h3 className="text-xl font-semibold text-gray-500 line-through opacity-70">{price}</h3>
